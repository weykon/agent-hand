> 本项目源自 Go 开源项目 [agent-deck](https://github.com/asheshgoplani/agent-deck) 的 Rust 重写版本（本仓库：**agent-hand**）。
...
- [x] **TUI 界面** - ratatui 实现
- [x] **搜索功能（TUI）** - 模糊搜索会话
- [ ] **MCP 管理（TUI）** - （目前已隐藏/禁用，后续会以更清晰的 UX 重新引入）
- [x] **会话分叉（TUI）** - 基于现有会话创建 fork
- [x] **tmux 快捷切换（Ctrl+G）** - 在 tmux 内弹出搜索窗口快速切换 session
...
```bash
# 添加会话（可选 -c/--cmd 用于启动 tmux 后运行的命令）
