...
impl TmuxManager {
    ...
    /// Expose server name for ptmx module
    ...
    /// Create a tmux session for viewing a remote shared session.
    ///
    /// The session runs `agent-hand pty-viewer` which bridges WebSocket ↔ stdio.
    /// Tmux renders PTY output natively — no vt100 parser needed on the viewer side.
    #[cfg(feature = "pro")]
    pub async fn create_viewer_session(
        ...
        let short_id = &room_id[..room_id.len().min(8)];
        let session_name = format!("viewer-{}", short_id);

        // Build the pty-viewer command
        ...
        if let Some(ut) = user_token {
            cmd_parts.push("--user-token".to_string());
            cmd_parts.push(ut.to_string());
        }
        if let Some(dn) = display_name {
            cmd_parts.push("--display-name".to_string());
            cmd_parts.push(dn.to_string());
        }
        ...
        // Create detached tmux session running the pty-viewer
        let output = self
            .tmux_cmd()
            .args([
                "new-session",
        ...
        // Set bindings
        self.ensure_server_bindings().await;
        self.register_session(session_name.clone());

        // Set display title
        let title = if let Some(host_name) = host_session_name {
            format!("👁 {}", host_name)
        } else {
            format!("👁 Room {}", short_id)
        };
        let _ = self.set_session_title(&session_name, &title).await;

        // Viewer-specific status-left showing host info and keybindings
        let host_label = host_session_name.unwrap_or(short_id);
        let viewer_status = format!(
            "👁 Viewing: {}  | Ctrl+G: info | Ctrl+Q: back",
            host_label
        );
        let _ = self
            .tmux_cmd()
        ...
        // Bind Ctrl+G to display info popup for this viewer session
        let info_cmd = format!(
            "'{}' viewer-info --room-id '{}'",
            bin.replace('\'', "'\\''"),
            room_id.replace('\'', "'\\''"),
    ...
    /// Send raw tmux keys WITHOUT appending Enter.
    pub async fn send_raw_keys(&self, name: &str, keys: &str) -> Result<()> {
        let output = self
            .tmux_cmd()
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(crate::Error::tmux(format!("send raw keys failed: {}", stderr)));
        }
        Ok(())
    }

    /// Send Escape to safely interrupt Claude Code (safer than C-c).
    pub async fn send_interrupt(&self, name: &str) -> Result<()> {
        self.send_raw_keys(name, "Escape").await
    }
