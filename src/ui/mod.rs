mod app;
pub mod canvas;
mod dialogs;
mod events;
mod input;
mod render;
mod switcher;
pub mod transition;

pub use app::App;
pub use dialogs::{
    CreateGroupDialog,
    DeleteConfirmDialog, DeleteGroupChoice, DeleteGroupDialog, Dialog,
    ForkDialog, ForkField, MoveGroupDialog, NewSessionDialog,
    NewSessionField, RenameGroupDialog, RenameSessionDialog, SessionEditField,
    SettingsDialog, SettingsField, SettingsTab,
    TagPickerDialog, TagSpec,
};

#[cfg(feature = "pro")]
pub use dialogs::{
    AnnotateDialog, ConfirmInjectionDialog, ContextInjectionMethod, ControlRequestDialog,
    CreateRelationshipDialog, CreateRelationshipField, CreateSkillField, CreateSkillWizard,
    DisconnectViewerDialog, HumanReviewDialog, InjectionTarget, InitSkillsWizard,
    InitWizardStatus, JoinSessionDialog, NewFromContextDialog, OrphanedRoomsDialog,
    ProposalActionDialog, ShareDialog, SkillCategory, SkillSection, SkillsBrowserEntry,
    SkillsCursor, SkillsManagerDialog, SkillsManagerMode,
};
#[cfg(feature = "max")]
pub use dialogs::{AiAnalysisDialog, AiAnalysisMode, BehaviorAnalysisDialog};

pub use input::TextInput;
pub use switcher::run_switcher;
pub use transition::StartupPhase;

use crossterm::event::{KeyCode, KeyModifiers};

/// UI events
#[derive(Debug, Clone, PartialEq)]
pub enum Event {
    Key(KeyCode, KeyModifiers),
    Tick,
    Resize(u16, u16),
    Quit,
}

/// Application state
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum AppState {
    /// Startup splash screen (ASCII logo animation)
    Startup,
    Normal,
    Search,
    Dialog,
    Help,
    #[cfg(feature = "pro")]
    Relationships,
    /// Viewing a shared terminal session via relay (Pro only).
    #[cfg(feature = "pro")]
    ViewerMode,
    /// Chat panel is focused — user is typing into the chat input.
    Chat,
}

#[derive(Debug, Clone)]
pub enum TreeItem {
    Group {
        path: String,
        name: String,
        depth: usize,
    },
    Session {
        id: String,
        depth: usize,
    },
    /// A session that was auto-created as a relationship workspace.
    Relationship {
        id: String,
        rel_id: String,
        depth: usize,
    },
}
