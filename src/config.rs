...
pub struct ConfigFile {
    ...
    /// Context bridge (guarded context injection) configuration
    #[serde(default)]
    pub context_bridge: ContextBridgeConfig,

    /// Enable canvas agent projection (visualize runtime state on canvas).
    #[serde(default = "default_true")]
    pub canvas_projection_enabled: bool,

    /// Enable transition animations and startup logo.
    #[serde(default)]
...
fn default_cb_scope() -> String {
    "self_only".to_string()
}
fn default_cb_trigger_events() -> Vec<String> {
    vec!["user_prompt_submit".to_string()]
}
fn default_cb_cooldown() -> u64 {
    5
}
fn default_cb_max_lines() -> usize {
    50
}
fn default_cb_max_chars() -> usize {
    10000
}

/// Configuration for the guarded context-injection pipeline.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct ContextBridgeConfig {
    /// Whether the context bridge is enabled at all.
    #[serde(default = "default_true")]
    pub enabled: bool,
    /// Injection scope: "off", "self_only", "same_group", "confirmed_relations".
    #[serde(default = "default_cb_scope")]
    pub scope: String,
    /// Which hook events trigger context injection.
    #[serde(default = "default_cb_trigger_events")]
    pub trigger_events: Vec<String>,
    /// Minimum seconds between injections for the same session.
    #[serde(default = "default_cb_cooldown")]
    pub cooldown_secs: u64,
    /// Maximum lines of progress to include in context.
    #[serde(default = "default_cb_max_lines")]
    pub max_lines: usize,
    /// Maximum total characters in the context file.
    #[serde(default = "default_cb_max_chars")]
    pub max_total_chars: usize,
    /// Write a debug log alongside audit files.
    #[serde(default)]
...
impl Default for ContextBridgeConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            scope: default_cb_scope(),
            trigger_events: default_cb_trigger_events(),
            cooldown_secs: default_cb_cooldown(),
            max_lines: default_cb_max_lines(),
            max_total_chars: default_cb_max_chars(),
...
/// Advertised capabilities of the active context transport.
///
/// Future transports (ACPX, WebSocket) will override these defaults.
/// Used by the pipeline to decide which features are available.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TransportCapabilities {
    /// Whether the transport supports sidecar feedback files.
    #[serde(default = "default_true")]
    pub supports_sidecar: bool,
    /// Whether the transport supports file-based context injection.
    #[serde(default = "default_true")]
    pub supports_file_context: bool,
    /// Whether the transport supports live interrupt signals.
    #[serde(default)]
...
/// Skills library configuration (Pro tier)
#[cfg(feature = "pro")]
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct SkillsConfig {
    /// GitHub repo URL override (auto-detected from registry if not set)
    #[serde(default)]
    pub repo_url: Option<String>,
    /// Auto-sync skills on TUI startup
    #[serde(default)]
    pub auto_sync: bool,
}
...
impl ConfigFile {
    ...
    pub fn animations_enabled(&self) -> bool {
        self.animations_enabled.unwrap_or(true)
    }
    ...
    pub fn context_bridge(&self) -> &ContextBridgeConfig {
        &self.context_bridge
    }

    pub fn set_keybinding(&mut self, action: &str, specs: &[KeySpec]) {
        let strings: Vec<String> = specs.iter().map(|k| format_key_spec(k)).collect();
        if strings.len() == 1 {
            self.keybindings.insert(action.to_string(), OneOrMany::One(strings.into_iter().next().unwrap()));
        } else {
            self.keybindings.insert(action.to_string(), OneOrMany::Many(strings));
        }
    }
...
impl KeyBindings {
    ...
    pub fn get_specs(&self, action: &str) -> Option<&[KeySpec]> {
        self.bindings.get(action).map(|v| v.as_slice())
    }

    pub fn set_binding(&mut self, action: &'static str, specs: Vec<KeySpec>) {
        self.bindings.insert(action, specs);
    }

    pub fn matches(&self, action: &'static str, code: &KeyCode, modifiers: KeyModifiers) -> bool {
        // Crossterm reports Shift+A as Char('A') + SHIFT, but keybindings store
        // uppercase letters as Char('A') + NONE. Strip the implicit SHIFT for
        // uppercase characters so the comparison works.
        let effective_modifiers = if let KeyCode::Char(c) = code {
        ...
        self.bindings.get(action).is_some_and(|v| {
            v.iter()
                .any(|k| &k.code == code && k.modifiers == effective_modifiers)
        })
    }
}

pub fn format_key_spec(k: &KeySpec) -> String {
